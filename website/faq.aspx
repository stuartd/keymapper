<%@ Page Language="C#" AutoEventWireup="true" CodeFile="faq.aspx.cs" 
Inherits="faq" MasterPageFile="keymapper.master" Title="Key Mapper FAQ" %>
...
<asp:Content ID="default" ContentPlaceHolderID="body" runat="server">
      <form id="form1" runat="server">
            ...
            <h3 id="contents" runat="server">Contents placeholder</h3>
         <!-- As toc ul runs on server, can't predict it's ID, so wrap it in a div. -->
            <div id="faqtop"></div>
        <div id="toc_container"><ul id ="toc" runat="server"></ul></div>
        <h3 class="question" runat="server">
            What does Key Mapper do?</h3>
        <p>
            Key Mapper allows you to disable a keyboard key or to remap it to act as 
            another key which isn&#39;t on your keyboard. In this faq, the term &quot;key mapping&quot; is 
            used to describe disabled and remapped keys.</p>
            <br />
        <p>
            Unlike other programs which do this, Key Mapper lets you set key mappings for each
            person who on your computer (as well as mappings which apply to all users). 
            These don&#39;t require Administrative rights, and you can log off and on again to set and cancel key mappings rather than having
            to restart your computer. This functionality was introduced in Windows XP, so Windows 2000 users can't have 
            per-user key mappings.</p>
        <h3 class="question" runat="server">
            Why would I want to disable or remap my keys?</h3>
        <p>
            The most common uses are disabling the Num Lock or Insert key - keys which most people never press 
            except accidentally, and which cause confusion and inconvenience when they are pressed.
            </p>
            <br />
            <p>
            Another common use is disabling Caps Lock, or perhaps remapping it to Shift, so 
            accidentally pressing Caps Lock doesn't turn on Caps Lock, but it's functionality is still partially preserved.
           An option would be to remap another key to Caps Lock - perhaps Scroll Lock, which only Excel power-users ever use.
            </p>
            <br />
            <p>
           You can also remap keys you don't use to keys which aren't on your keyboard - Volume Up and Down, Play and Pause, Email.
            ...
            <br />
            <p>
                If you using virtualisation to run Windows on a Mac, you will have noticed that Mac keyboards
            don't have many of the keys Windows uses, for example Print Screen and Scroll Lock, and Key Mapper lets you 
            map keys to these, as well as remapping the Command key to Control and choosing which keys activate the context menu. 
                This also applies when using Remote Desktop For Mac to administer a Windows 
                computer remotely. (If you use Remote Desktop for Mac Beta 3 on a Mac with a UK 
                keyboard layout, disabling both Alt keys stops the annoying problem with the Alt 
                key &#39;sticking&#39; when pressed)</p>
            <br />
            <p>If your laptop keyboard has a broken key, you can use Key Mapper to map another key to the broken one rather than buy and fit a new keyboard.
            </p>
    <h3 class="question" runat="server">
        How do I disable or remap keys with Key Mapper?</h3>
    <p> To disable a key you can see on the virtual keyboard, drag and drop it off the virtual
        keyboard.</p>
    <br />
    <p>
       Also, if you want to map a key to another and they&#39;re both on the virtual keyboard, you can
        drag and drop the action key onto the target key. For example, to map Caps Lock
        to act like Left Shift, drag and drop the Left Shift key onto the Caps Lock key.
        </p>
        <br />
    <p>If you want to remap a key on the virtual keyboard to a key that isn&#39;t on the virtual keyboard,
    double-click the key to bring up the Add Mapping screen. From here, you can choose
    what you want the key to do from the lists presented.</p>
    <br />
    <p>
        The key lists are divided into three groups:</p>
    <ul class="sublist">
        <li>Useful Keys - these are keys which are commonly used - and most useful - in key mappings.</li>
        ...
        <li>All Keys - these are all the keys which are available. Some of these require special
            keyboard drivers to work, and some cannot be used in key mappings because of the way
        ...
        <br />
    <ul class="sublist">
        <li>To select from the lists, choose &quot;Create New Mapping&quot; from the Mappings menu, 
            and then the &quot;Select From Lists&quot; option. The keys are arranged into groups, with 
            &#39;All keys&#39; as the first group. Find the key you want to remap, then click Set.</li>
        <li>To use Key Capture, choose "Create New Mapping" from the Mappings menu, and then the
            "Use Key Capture" option. You&#39;ll see a blank key: press the key you want to remap.
            ...
            aren&#39;t received by Windows: these keys can&#39;t be remapped or disabled.</li>
    </ul>
    <br />
    <p>
    ...
    <p>
        If the remapped or disabled key is shown on the virtual keyboard, simply drag it off
        the keyboard to restore or enable it.</p>
    <p>
        If the remapped or disabled key isn&#39;t on the virtual keyboard, you can delete the key mapping from the
        Mapping List screen. Click on Mapping List in the keyboard&#39;s Windows Menu to
        show the screen: it shows all your current and pending key mappings, and allows you to
        delete key mappings by clicking the Delete column.</p>
    <p>
        You can also clear all your key mappings by choosing "Clear All Mappings" from the Mappings
        menu.</p>
    <h3 class="question" runat="server">
        How do I undo my changes?</h3>
    Key Mapper supports Undo and Redo from the Edit menu: you can also restore your
    key mappings to their current effective state by choosing Revert To Saved Mappings from
    the Mappings menu (this restores the key mappings shown to those which are currently
    in effect, discarding any new key mappings and restoring any cleared key mappings)<p>
        You can also clear all your key mappings by choosing Clear All Mappings from the Mappings
        menu.</p>
    ...
    <p>
        There are some key mappings to non-standard keyboard keys that can be useful: how well they work
        depends on which program you use for music and web browsing.</p>
    <ul class="sublist">
        <li>Volume Keys - you can remap keys to Volume Up, Volume Down, and Mute. These are controlled
        ...
        <li>Media Keys - Next Track, Previous Track, Play / Pause, and Stop are available, These
            <a href="http://msdn.microsoft.com/en-us/library/ms997498.aspx#mshrdwre_topic4">should</a> be recognised
            by any mainstream music player: some players - iTunes, for example - will only react
            to keys when it is the frontmost application; and some - like
            VLC - ignore them altogether, 
        ...
        <li>Browser Keys - Back, Forward, Stop Loading, Refresh, and Home can be remapped to 
            (if you use Internet Explorer, you can also remap a key to your &#39;Favourites&#39;). A 
            key remapped to Home will start your browser if isn&#39;t already running, and 
            start a new browser window if it is. If you press it while your browser is the 
            current window, you will return to your home page.</li>
    ...
    </ul><h3 class="question" runat="server">
        I remapped a function key and now I can&#39;t use it anymore in another program where 
        I need it!</h3>
<p>
    ...
    <p>
        By default, remapped and disabled keys only apply to the current user (except in Windows
        2000, which only allows key mappings which apply to all users). To set key mappings which
        ...
        and create the required key mappings. You can have both boot key mappings and user key mappings:
        if the same key is remapped in each, then user key mappings override boot key mappings. If
        you use Fast User Switching, then you will need to use Boot Mappings as User Mappings
        are discarded when using Fast User Switching to switch to an account that is
        already logged on.</p>
        <br />
    <p>
    ...
    <p>
        ...
        be vertical) you can switch it by clicking "Change Enter Key@ on the Keyboard menu, and 
        you can click "Always Use This Enter Key Orientation For This Layout" on the Keyboard menu to save the 
        change.
        <br />
        If you have multiple keyboards installed, the displayed keyboard should be the same language as your current keyboard unless you've changed your keyboard since Key Mapper started.</p>
<p>
        The Keyboard Menu is also the place to specify you have a Mac keyboard (where the Alt
        and Windows (Command) keys are reversed), or switch the Number Pad off if you have
        a laptop.</p>
        
          <h3 class="question" runat="server">How do I see all the key mappings I've created?</h3>
    <p>
    ...
    <p>
        ...
        other languages doesn&#39;t work on Chinese or Korean keyboards.</p>
    <h3 class="question" runat="server">
    ...
    <p>
        ...
        it depends how it's configured.</p>
        <p>
        ...
        will be installed when you click OK or Apply so have your CD (or the path to your i386 folder)
         handy, and you'll need to restart your computer.</p>
        <h3 class="question" runat="server">
            Can I remap SHIFT-4 or CTRL-Q to another character?</h3>
        <p>
            No. Scancode key mappings apply to the physical key - Shift-4 is two keypresses, Shift
            ...
            There are some quirky key combinations which can be remapped though, due to the way Windows
            processes keypresses - you can remap Alt-PrtScr and Ctrl-Break, for example (Alt PrtScr
    ...
    <h3 class="question" runat="server">
        I&#39;ve set up a boot key mapping to disable Num Lock but when I need to log in Num Lock 
        is off. How can I set it to come on?</h3>
    <p>
        Use the "Set Current Toggle Keys As Default" item on the Toggle Keys menu to 
        set the keys at login.</p>
    ...
    <p>
        ...
        to <a href="http://code.google.com/p/keymapper/source/browse/trunk/keymapper">browse</a> or <a href="http://code.google.com/p/keymapper/downloads/list">
        ...
        you can use Microsoft&#39;s free <a href="http://www.microsoft.com/express/vcsharp/">
            ...
            <h3 class="question" runat="server">Why does Key Mapper disable Num Lock when I remap my Pause key?</h3>

            <p>Key Mapper automatically disables Num Lock when you remap your Pause key, as otherwise Num Lock will 
            toggle whenever you press the remaped Pause key. Even if you have Num Lock remapped to another key, 
            it will still fire when you press the remapped Pause key, which could be very annoying: a 
                useful side-effect of this though is that you can
            assign two keystrokes 
            to the Pause key: the first is what you remap Pause to, and the second is what you remap Num Lock to.
            <br /><br />
            In order to do this, you have to remap Pause to the first key in the combination, let Key Mapper disable Num Lock, then 
            re-enable Num Lock and map it to the second key in the key combination.<br /><br /> 
          
            For example: remapping Pause to Alt-F4:<br />
            Map Pause => Alt<br />
            Num Lock: => 4<br />
            <br />
            Pause is thus remapped to Alt-F4, the shortcut used to close programs (or if the desktop has the focus, restart Windows)
            <br /><br />
            Another example: remapping Pause to the Euro symbol<br />
            Pause => Right Alt<br />
            Num Lock => 4 $<br />
            <br />
            Pause is remapped to Right-Alt-4, the shortcut for the Euro sign.
            <br /><br />
            Other possibilities would be:<br />
            Shift-F10 (Context menu)<br />
            Shift 2 (or Shift ' depending on keyboard layout) to get @<br />
            Windows-L (Lock Workstation in Windows XP or later)<br />
            <br />
            </p>
            <h3 class="question" runat="server">
            I've remapped some keys, but my keyboard hasn't been updated and still shows the original keys!</h3>
            <p>This functionality will have to wait for a future version. (Thanks to Steve Ward for this question.)</p>
</form>
</asp:Content>
